...
parser.add_argument('--lr', '--learning-rate', default=0.001, type=float, required=False, help='(default=%(default)f)')
...
parser.add_argument('--print_freq', default=2, type=int, required=False, help='(default=%(default)d)')
parser.add_argument('--save_freq', default=2, type=int, required=False, help='(default=%(default)d)')
...
def main():
    ...
    # Data loading code
    train_dataset, val_dataset, attr_num, description,_ = Get_Dataset(args.experiment, args.approach)
    ...
    # create model
    model = models.__dict__[args.approach](pretrained=False, num_classes=attr_num)
    ...
    criterion = Weighted_BCELoss('None')
...
def validate(val_loader, model, criterion, epoch):
    ...
    for i, _ in enumerate(val_loader):
        input, target = _
        # print("valtarget{}".format(len(target)))
        target = target.cuda(non_blocking=True)
        input = input.cuda(non_blocking=True)
        output = model(input)
...
class Weighted_BCELoss(object):
    ...
    def forward(self, output, target, epoch):
        if self.weights is not None:
            ...
            # print("lossfunc{}".format(target.shape))
