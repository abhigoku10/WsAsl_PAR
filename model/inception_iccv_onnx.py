...
class ChannelAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = F.avg_pool2d(x, x.size()[2:])# commenting because trying toe convert to onnx
...
class SpatialTransformBlock(nn.Module):
    ...
    def forward(self, features):
        ...
        for i in range(self.num_classes):
            ...
            stn_shape = [int(s) for s in stn_feature.shape[2:]]

            theta_i = self.stn_list[i](F.avg_pool2d(stn_feature, stn_shape).view(bs,-1)).view(-1,4)
            ...
            theta_class.append(theta_i)
            # sub_feature_class.append(sub_feature)
        pred = torch.cat(pred_list, 1)
        return pred ,theta_class#,sub_feature_class
...
class InceptionNet(nn.Module):
    ...
    def forward(self, input):
        ...
        pred_3b,theta3b = self.st_3b(fusion_3b) #,sub_feature_3b
        pred_4d,theta4d = self.st_4d(fusion_4d) #,sub_feature_4d
        pred_5b,theta5b = self.st_5b(fusion_5b) #,sub_feature_5b

        return pred_3b, pred_4d, pred_5b, main_pred,theta3b,theta4d,theta5b  #,sub_feature_3b,sub_feature_4d,sub_feature_5b
